    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt):
                if insect.kween == True and insect.imposter == False:
                    return None
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        spot = self.place
        moving = 0
        while spot != beehive and moving <= self.max_range:
            if spot.bees:
                return random_or_none(spot.bees)
            spot, moving = spot.entrance, moving + 1
        return None
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter == True:
            ...
            while check_place != None:
                if check_place.ant in self.list_doubled and check_place.ant.is_container:
                    if check_place.ant.contained_ant is not None and check_place.ant.contained_ant not in self.list_doubled:
                        check_place.ant.contained_ant.damage*=2
                        self.list_doubled.append(check_place.ant.contained_ant)


                if check_place.ant and check_place.ant not in self.list_doubled:
                    check_place.ant.damage *= 2
                    self.list_doubled.append(check_place.ant)
                    if check_place.ant.is_container:
                        if check_place.ant.contained_ant and not isinstance(check_place.ant.contained_ant, QueenAnt) and (check_place.ant.contained_ant not in self.list_doubled):
                            check_place.ant.contained_ant.damage *= 2
                            self.list_doubled.append(check_place.ant.contained_ant)
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Insect.reduce_armor(self, amount)
        if self.kween and not self.imposter:
            bees_win()
        # END Problem 13
